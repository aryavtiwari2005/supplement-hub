...
import { useTheme } from "@/components/ThemeProvider"; // Assuming this is the correct path
...
export default function LoginPage() {
  ...
  const { theme } = useTheme(); // Using useTheme from ThemeProvider
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Dispatch authChange event to notify other components (e.g., ProfileDropdown)
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center p-4 pt-20 ${THEMES[theme].background.primary}`}
    >
      <motion.div
